...
const Product = ({ item, provider, account, crowdfunding, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    const x = parseInt(item.noOfContributions.toString());

    const contributors = [];
    const contributions = [];
    for (var i = 0; i < x; i++) {
      const y = await crowdfunding.campaignContributions(
        parseInt(item.ID.toString()),
        i
      );
      const contributor = y.contributorAddress;
      contributors.push(contributor);
      const contribution = y.amtContributed;
      contributions.push(
        ethers.utils.formatUnits(contribution.toString(), "ether")
      );
      myMap.has(contributor)? myMap.set(contributor, myMap.get(contributor) + parseInt(ethers.utils.formatUnits(contribution.toString(), "ether"))) :
      myMap.set(contributor, parseInt(ethers.utils.formatUnits(contribution.toString(), "ether")))
    }
  ...
  const buyHandler = async () => {
    ...
    var etherAmount = web3.utils.toBN(input);
            ...
            {contributors&&contributions && (
              <ul>
                {contributors.map((contributor) => (
                  <li> {contributor.slice(0,5) + "..." + contributor.slice(25,30)} : {}</li>
                ))}
              </ul>
            )}
          </div>
