...
const Product = ({ item, provider, account, crowdfunding, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    for (var i = 0; i < x; i++) {
      ...
      myMap.has(contributor)
        ? myMap.set(
            contributor,
            myMap.get(contributor) +
              parseInt(
                ethers.utils.formatUnits(contribution.toString(), "ether")
              )
          )
        : myMap.set(
            contributor,
            parseInt(ethers.utils.formatUnits(contribution.toString(), "ether"))
          );
    }
  ...
  return (
    <div className="product" style = {myComponent}>
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          {manager && (
            <h3>{manager.slice(0, 5) + "..." + manager.slice(25, 30)}</h3>
          )}
          <br />
          <h3>
              ...
              <ul>
                {
                  contributors.map((item, i) => {
                    return <li key = {i}>{item.slice(0,5) + "..." + item.slice(25,30)} : {contributions[i]}</li>
                  })
                }
              </ul>
            )}
