...
const Section = ({ title, items, togglePop }) => {
  ...
  const handleVote = async (item) => {
    ...
    console.log(item.voteCount.toString())
    const noOfCampaigns = await contract.methods.noOfCampaigns().call();
    const total = parseInt(noOfCampaigns.toString());
    let totalVotes = 0;
    for(var i=0; i<total; i++)
    {
      totalVotes += parseInt(items[i].voteCount.toString())
    }
    
    let votes = []
    for(var i=0; i<total;i++)
    {
      votes[i]=(parseInt(item.voteCount.toString())/ totalVotes);
    }
    setVotes(votes);
    console.log(totalVotes);
    console.log(votes[parseInt(item.ID.toString())])
  };

  const fetchDetails = s
  useEffect(() => {
          ...
          <Card sx={{ maxWidth: 345 }}>
            ...
            >
              ...
              <CardContent>
                ...
                <br />
                {/* <Rating name="read-only" value={votes[parseInt(item.ID.toString())]} readOnly /> */}
                <br />
                <h4>
                  Goal Amount :{" "}
