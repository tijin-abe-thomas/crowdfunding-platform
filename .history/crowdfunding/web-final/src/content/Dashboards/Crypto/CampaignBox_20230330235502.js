...
const Product = ({ item, provider, account, crowdfunding, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    for(var i=0; i< x; i++)
    {
      const y = await crowdfunding.campaignContributions(parseInt(item.ID.toString()) , i)
      const contributor = y.contributorAddress
      contributors.push(contributor)
      const contribution = y.amtContributed
      contributions.push(ethers.utils.formatUnits(contribution.toString(), "ether"))
    }
      ...
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <div>
            ...
            <ul>{contributors.map(contributor => <li key = {contributor}> {contributor} </li>)}</ul>
          </div>
          <br />
          {contribution && (
