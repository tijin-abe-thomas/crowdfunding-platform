...
function CreateCampaign() {
  ...
  const handleSubmit = async () => {
    ...
    const minContribution = ethers.utils.parseUnits(mincontribution, 'ether');

    const goalInput = document.getElementById(
      "goal-campaign"
    ) as HTMLInputElement;
    const Goal = goalInput.value;
    const goal = ethers.utils.parseUnits(Goal, 'ether');

    const deadlineInput = document.getElementById(
      "deadline-campaign"
    ) as HTMLInputElement;
    const Deadline = parseInt(deadlineInput.value)*86400 + Date.now();
    const deadline = new BigNumber(Deadline).multipliedBy(BigNumber(86400)).plus(BigNumber(Date.now()));
