...
function CreateCampaign() {
  ...
  const handleSubmit = async () => {
    ...
    const minContribution = ethers.utils.parseUnits(mincontribution.toString(), 'ether');

    const goalInput = document.getElementById(
      "goal-campaign"
    ) as HTMLInputElement;
    const Goal = goalInput.value;
    const goal = ethers.utils.parseUnits(Goal.toString(), 'ether');

    const deadlineInput = document.getElementById(
      "deadline-campaign"
    ) as HTMLInputElement;
    const Deadline = parseInt(deadlineInput.value)*86400;
