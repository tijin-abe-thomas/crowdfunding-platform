...
async function createCampaign(
  ...
  await contract.methods
    .uploadCampaign(
      ...
      location,
      category,
      ethers.utils.parseUnits(minContribution, 'ether'),
      ethers.utils.parseUnits(goal, 'ether'),
      deadline
    )
...
function CreateCampaign() {
  ...
  const handleSubmit = async () => {
    ...
    const ntemp = parseInt(noOfCampaigns.toString()) +1 
    const total = new BigNumber((ntemp.toString()));
    console.log(total);
    ...
    const Deadline = parseInt(deadlineInput.value) * 86400000 + Date.now();
    console.log("hello", Deadline, Date.now());
