...
const contract = new web3.eth.Contract(contractAbi, '0x7af2a8EA9a52Dddc5632E00192050bFd636F6974');

async function createCampaign(id: string, title: string, description: string, location: string, category: string, minContribution: BigNumber, goal: BigNumber, deadline: BigNumber): Promise<void> {
  const accounts = await web3.eth.getAccounts(); // get the current user's account
  await contract.methods.uploadCampaign(id, title, description, location, category, minContribution, goal, deadline).send({ from: accounts[0], gas:500000}); // call the setName function and send the transaction
}
...
function CreateCampaign() {
  ...
  const handleSubmit = async () => {
    ...
    const titleInput = document.getElementById('title-campaign') as HTMLInputElement
    const title = titleInput.value;

    const descriptionInput = document.getElementById('description-campaign') as HTMLInputElement
    const description = descriptionInput.value;

    const locationInput = document.getElementById('location-campaign') as HTMLInputElement
    const location = locationInput.value;

    const categoryInput = document.getElementById('category-campaign') as HTMLInputElement
    const category = categoryInput.value;

    const mincontributionInput = document.getElementById('minContribution-campaign') as HTMLInputElement
    const mincontribution = mincontributionInput.value;
    const minContribution = new BigNumber(mincontribution);

    const goalInput = document.getElementById('goal-campaign') as HTMLInputElement
    const Goal = goalInput.value;
    const goal = new BigNumber(Goal);

    const deadlineInput = document.getElementById('deadline-campaign') as HTMLInputElement
    const Deadline = deadlineInput.value;
    const deadline = new BigNumber(Deadline);

    createCampaign(id, title, description, location, category, minContribution, goal, deadline)
  };
